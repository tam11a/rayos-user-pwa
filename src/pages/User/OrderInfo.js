...
const OrderInfo = () => {
	...
	const [forReview, setForReview] = React.useState(undefined);
	...
	return (
		<>
			{forReview && (
				<Review
					onCloseReview={() => setForReview()}
					orderId={oid}
					orderLine={forReview?._id}
					product={forReview?.product?._id}
				/>
			)}
			<Container>
				<Grid
										...
										>
											<ListItemAvatar>
												<Avatar
													...
													component={Link}
													to={`/product/${prodItem?.product?._id}`}
												>
													<IoIosImages
														style={{
															fontSize: "1.8em",
